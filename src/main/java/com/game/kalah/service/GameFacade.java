	...
	public void makeMove(Game game, int pitId) {
	    ...
	    private void validatePitNumber(int pitId, Game game) {
	        ...
	        if (pitId == player.getKalahId() || pitId == player.getOppositePlayer().getKalahId()) {
	            throw new InvalidIdException("You can not select Kalah Home!");
	        }

	        if (pitId < FIRST_PIT_INDEX || pitId > LAST_PIT_INDEX) {
	        	System.out.println("Provided pitId is out of bounds...");
	            throw new InvalidIdException("Provided pitId is out of bounds...");
	        }

	        if (!isUserPit(pitId, player)) {
	        	System.out.println("It is not your turn!");
	            throw new InvalidIdException("It is not your turn!");
	        }
	        if (game.getScoreBoard().get(pitId) == 0) {
	        	System.out.println("You can not select empty pit!");
	            throw new InvalidIdException("You can not select empty pit!");
	        }
	    }
